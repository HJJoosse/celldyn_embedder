...
class Embedding:
    ...
    def umap_embedding(
        self, dimensions: int, n_neighbors: int = 30, min_dist: int = 0.0
    ):
    ...
    def trimap_embedder(
        self, dimensions: int, n_inliers: int, n_outliers: int, n_random: int
    ):
    ...
    def pacmap_embedder(
        self, dimensions: int, n_neighbors: int, distance: str = "euclidean"
    ):
        self.embedder = pacmap.PaCMAP(
            n_dims=dimensions, n_neighbors=n_neighbors, distance=distance
        )
    ...
    def clean_embedding(
        self, embedding, n_samples=1000000, n_workers=8, radius=1, n_neighbors=100
    ):
        ...
        self.embedding_df = pd.DataFrame(
            data=embedding, index=np.arange(0, len(embedding)), columns=columns
        )

        split_embedding = self.chunk_it(embedding, n_workers)
        pool = ThreadPool(n_workers)
        results = [
            pool.apply_async(self.clean_single, (subset, columns, radius, n_neighbors))
            for subset in split_embedding
        ]
        pool.close()
        pool.join()
...
class MaximumVarianceUnfolding(BaseEstimator, TransformerMixin):
    ...
    def fit(self, data, y=None):
        ...
        N = KNN(n_neighbors=self.n_neighbors).fit(data).kneighbors_graph(data).todense()
        N = np.array(N)
...
class LandmarkMaximumVarianceUnfolding(BaseEstimator, TransformerMixin):
    ...
    def fit(self, data, y=None):
        ...
        L = (
            KNN(n_neighbors=3)
            .fit(top_landmarks)
            .kneighbors_graph(top_landmarks)
            .todense()
        )
        L = np.array(L)
        ...
        l = KNN(n_neighbors=3).fit(top_landmarks).kneighbors_graph(new_data).todense()
        l = np.array(l)
...
class LandmarkMultiDimensionalScaling(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, X, y=0):
        ...
        N = self.D.shape[1]
        Lh = V.dot(np.diag(1.0 / np.sqrt(self.evals[w]))).T

...
def random_seach_optimization(X, model, evaluators, param_grid ,ascending,subsampling = False,seperating_param_values =False, max_evals = 300, num_iter = 5, random_state = None, **kwargs):
    ...
    **kwargs:
        Whatever key words that are allow in the model 
    ...
    for i in range(max_evals):
        ...
        for num in range(num_iter):
            ...
            predicted = None
            if('metric' in hyperparameters.keys()):
                if(hyperparameters['metric'] == 'mahalanobis'):
                    V=np.cov(sub, rowvar=False)
                    predicted = model(**hyperparameters, V = V).fit_predict(sub)
                else:
...
def grid_seach_optimization(X, model, evaluators, param_grid ,ascending,subsampling = False, seperating_param_values =False,num_iter = 5, random_state = None, **kwargs):

...
# cast in sklearn api
    ...
    def qc_leuko(self, df: pd.DataFrame):
        ...
        for chan in ['na', 'ni', 'nd', 'nf', 'np', 'li', 'la']:
            try:
                df.loc[df[f"c_b_{chan}cv"] < 0.00000000000001, [f"c_b_{chan}cv",f"c_b_{chan}mn"]] = np.nan
            except KeyError:
    ...
    def qc_rbc(self, df: pd.DataFrame):
        ...
        change_cols = ["c_b_retc","c_b_pretc","c_b_irf","c_b_rbcimn","c_b_rbcicv","c_b_rbcfmn","c_b_rbcfcv",
                                "c_b_mchcr","c_b_hdw","c_b_mchr","c_b_mcvr","c_b_phpo","c_b_phpr"]
        change_cols = list(set(change_cols).intersection(set(temp_cols)))

        conds = {'c_mode_rtc': ('==', 0, np.nan),
        ...
        conds = {'c_b_rbcicv': ('<', 0.0001, np.nan),
        ...
        change_cols = ["c_b_mchcr","c_b_hdw","c_b_mchr","c_b_mcvr","c_b_phpo","c_b_phpr"]

        conds = {'c_b_mchr': ('<', 0.0001, np.nan),
    ...
    def qc_standard_values(self, df: pd.DataFrame):
        ...
        real_cols = [c for c in df]
        df.columns = temp_cols
        standard_vals = {"c_b_rbcimn":182,"c_b_rbcicv":1.59341,"c_b_rbcfmn":85,"c_b_rbcfcv":7.2,
                         "c_b_namn":140, "c_b_nimn":150, "c_b_npmn":125, "c_b_ndmn":28, "c_b_nfmn":69,
                         "c_b_lamn":100, "c_b_limn":75, "c_b_hb":6.206e-21, "c_b_mch":0.6206, "c_b_mchc":0.6202}
